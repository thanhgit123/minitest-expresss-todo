...
export default function Todo() {
  ...
  const handleGetData = async () => {
    ...
    const taskPerPage = 4;
    try {
      const res = await axios.get(`http://localhost:7000/api/v1/todos?per_page=${taskPerPage}`);
      setAllTodo(res.data);
    } catch (error) {
      console.log(error);
    }finally{
...
// change completed to line-through
  const handleChangeCompleted = async (item)=>{
      try {
        const res = await axios.patch(`http://localhost:7000/api/v1/todos/${item.id}`,item);
        setFlag(!flag)
      } catch (error) {
        console.log(error);
      }
        ...
        <div className="bg-gradient-to-r from-emerald-500 via-purple-600 to-pink-500  pb-6 h-[500px] w-[500px] m-auto mt-4 pt-5  leading-10">
          ...
          {allToDo.map((item, index) => {
            return (
              <div key={index} className="pt-5 leading-10 flex">
                <div className="bg-white w-[370px] ml-5 h-[40px] rounded-[3px] pl-5 cursor-pointer" onClick={()=>handleChangeCompleted(item)}>
                  {" "}
          ...
          <div className=" pt-7 leading-10 flex">
            <div className="bg-white w-[370px] ml-5 h-[40px] rounded-[3px] pl-5">
              You have <span className="text-red-400">{allToDo.length}</span>{" "}
              pending task !
            </div>
            <button className="bg-sky-200 w-[60px] rounded-[3px] ml-4 " onClick={handleClearAll}>
              Clear All
            </button>
          </div>
        </div>
